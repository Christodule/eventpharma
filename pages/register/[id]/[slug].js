...
const RegisterPage = ({ event }) => {
	...
	if (loading) {
		return <Loading title='Generez votre ticket' />;
	}
	if (!event.title) {
		return <ErrorPage />;
	}
	...
	return (
		<div>
			<Head>
				<title>{`${event.title} | Cepharm Event`}</title>
				<meta
					name='description'
					...
					<div className='w-full h-[100vh] dim absolute top-0 flex items-center justify-center z-40'>
						<div className='w-[400px] bg-white h-[300px] flex items-center justify-center flex-col rounded-md shadow-[#FFD95A] shadow-md'>
							...
							<p className='text-center mb-6'>
								Verifiez votre email pour voir votre ticket et les informations de l'événement.
							</p>
							<button
